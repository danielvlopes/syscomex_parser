...
describe Syscomex::RowParser do
  ...
  context "syscomex spec based rows" do
    ...
    it 'extracts alphanumeric portions' do
      extracted, rest = Syscomex::RowParser.new(data_sample, [:text, 10]).extract
    ...
    it 'extracts numeric portions' do
      extracted, rest = Syscomex::RowParser.new(data_sample, [:numeric, 5]).extract
    ...
    context "extracting boolean" do
      it 'is true when S' do
        ...
        extracted, rest = Syscomex::RowParser.new(data_sample, [:boolean, 1]).extract
